...
uint8_t timedjob_add(uint8_t times, uint16_t interval10ms, uint8_t opts, jobf_t func)
{
	for(uint8_t i = 0; i < TIMEDJOB_MAX_ENTRIES; i++)
	{
		if(~timedjoblist[i].cOpts & TIMEDJOB_OPT_INUSE)
		{
			...
			timedjoblist[i].cInterval = interval10ms;
...
void timedjob_set_interval(uint8_t jobid, uint16_t interval10ms)
{
	timedjoblist[jobid].cInterval = interval10ms;
}
